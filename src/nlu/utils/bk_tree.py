...
__all__ = ['hamming_distance', 'BKTree', 'edit_distance']
...
def edit_distance(s, t, cache=None):
    ...
    if s[0] == t[0]:
        ...
        d = edit_distance(s[1:], t[1:], cache)
    else:
        # First chars not equal, it's 1 + edit distance of the rest
        d = 1 + min(
            edit_distance(s[1:], t[1:], cache),  # substitution of first chars
            edit_distance(s[1:], t, cache),      # delete first char of s
            edit_distance(s, t[1:], cache),      # delete first char of t (addition)
        )
...
if __name__ == '__main__':
    tree = BKTree(edit_distance, ['hédézhì', 'dézhì', '沈岗', '刘勇星', '刘苗', '刘星勇'])
    print(tree.find('刘勇', 0))
