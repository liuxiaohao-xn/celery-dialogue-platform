...
# @File : sec_action_not_complete.py
import random
import cn2an
import copy
...
from src.dm.actions.action import SlotMissingAction, EntityErrorAction, EntityRepeatAction, SuccessAction, \
    EntityFollowUpAction, EntityAction
...
class MeetingTimeErrorAction(EntityAction):

    def run(self, msg: SysMsg, slot: Slot, status: Text, select_date: List[Tuple], flow: Flow) -> SysMsg:
        from src.dm.dialogue_flow.meeting_time_dialogue_flow.meeting_time_verify_dialogue_flow \
            import MeetingTimeActionStatus
        if status == MeetingTimeActionStatus.MEETING_TIME_OUT_MODE:
            msg.rsp = f"会议时间为过去时间, 请再说下会议时间."
        elif status == MeetingTimeActionStatus.MEETING_TIME_DATA_NOT_ALIGN_WEEK:
            msg.rsp = f"日期星期错误, 请再说下会议时间."
        elif status == MeetingTimeActionStatus.MEETING_TIME_CONFIRM:
            select_d = (str(date[-1]) + "号" for date in select_date)
            select_rsp = "还是".join(select_d)
            msg.rsp = f"请问是{select_rsp}呢?"
        elif status == MeetingTimeActionStatus.MEETING_TIME_NOT_COMPLETION:
            msg.rsp = f"完成了"
        return msg
